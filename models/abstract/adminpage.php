...
abstract class RPBCalendarAbstractModelAdminPage extends RPBCalendarAbstractModel
{
	...
	private function initializeSelectedSubPageInfo()
	{
		...
		// Regular case => use the GET parameter `rpbcalendar_subpage` or the default subpage name
		// to determine the currently selected sub-page.
		if(isset($this->subPages))
		{
			// Filter the GET parameter 'rpbcalendar_subpage' to ensure that it actually corresponds
			// to a sub-page name.
			if(isset($_GET['rpbcalendar_subpage'])) {
				$target = $_GET['rpbcalendar_subpage'];
				$valid  = false;
				foreach($this->subPages as $subPage) {
			...
			// Use the default sub-page if the GET parameter 'rpbcalendar_subpage'
	...
	{
		global $pagenow;
		return admin_url($pagenow) . '?page=' . urlencode($_GET['page']) . '&rpbcalendar_subpage=' . urlencode($subPageName);
	}
